...
describe("NFTMarketplace", function () {
  ...
  describe("Check admin address", function(){
    it("Should track admin address", async function() {
  ...
  describe("Making marketplace items", function () { 
    ...
    it("Should track newly created item, track owner of NFT is addr1", async function () {
    ...
    it("Should track newly created item, track owner of NFT is marketplace", async function () {
      // addr1 offers their nft at a price of 1 ether
      await expect(marketplace.connect(addr1).makeItem(nft.address, 1))
        .to.emit(marketplace, "Offered")
        .withArgs(
      ...
      await (marketplace.connect(addr1).transItem(nft.address, 1));
  ...
  describe("Start auction", function () { 
    ...
    beforeEach(async function () {
      ...
      // addr1 approves marketplace to spend nft
      await nft.connect(addr1).setApprovalForAll(marketplace.address, true)
      expect(await marketplace.connect(addr1).makeItem(nft.address, 1 ))
      await (marketplace.connect(addr1).transItem(nft.address, 1));
    ...
    it("Should start auction", async function () {
      ...
      await expect (marketplace.connect(addr1).startAuction(1, 1000, toWei(startPrice))).to.emit(marketplace, "StartAuction").withArgs(true,toWei(startPrice));
      const item = await marketplace.items(1)
  ...
  describe("Bid item", function () {
    ...
    beforeEach(async function () {
      ...
      await marketplace.connect(addr1).startAuction(1, 1000000000, toWei(price));      
    })
    ...
    it("Should re-update highest bidder as address 2 and highest bid as toWei(5)", async function() {
      await expect(marketplace.connect(addr2).bid(1, {value: toWei(2)})).to.emit(marketplace, "Bid").withArgs(1, addr2.address, toWei(2));
      await expect(marketplace.connect(addr3).bid(1, {value: toWei(3)})).to.emit(marketplace, "Bid").withArgs(1, addr3.address, toWei(3));
      await expect(marketplace.connect(addr2).bid(1, {value: toWei(5)})).to.emit(marketplace, "Bid").withArgs(1, addr2.address, toWei(7));
      const item = await marketplace.items(1)
      expect(+fromWei(item.highestBid)).to.equal(7)
      expect(item.highestBidder).to.equal(addr2.address)
    })

    it("Should re-update highest bidder as address 2 and highest bid as toWei(5) part 2", async function() {
      await expect(marketplace.connect(addr2).bid(1, {value: toWei(2)})).to.emit(marketplace, "Bid").withArgs(1, addr2.address, toWei(2));
      await expect(marketplace.connect(addr3).bid(1, {value: toWei(4)})).to.emit(marketplace, "Bid").withArgs(1, addr3.address, toWei(4));
      await expect(marketplace.connect(addr2).bid(1, {value: toWei(3)})).to.emit(marketplace, "Bid").withArgs(1, addr2.address, toWei(5));
      const item = await marketplace.items(1)
    ...
    it("Should not bid if aution timeout", async function() {
      await marketplace.connect(addr1).startAuction(1, 1, toWei(price));    //time out in 1 sec  
  ...
  describe("End Auction", function() {
    ...
    beforeEach(async function () {
      ...
      await marketplace.connect(addr1).startAuction(1, 100000, toWei(price));      
    })

    // it("Should require auction start", async function() {
    //   await expect(marketplace.end(1)).to.be.revertedWith("not ended");
    // })
    it("Should not return null if there is no one bid", async function() {
      ...
      const item = await marketplace.items(1);
      expect(item.isStarted).to.equal(false)
      expect((+fromWei(sellerInitalEthBal)).toFixed(2)).to.equal((+fromWei(sellerFinalEthBal)).toFixed(2)) 
      expect(await nft.ownerOf(1)).to.equal(marketplace.address);
    ...
    it("Should transform NFT to highest bidder and trans ETH to seller", async function() {
      ...
      expect((+fromWei(item.highestBid)+ +fromWei(sellerInitalEthBal)).toFixed(2)).to.equal((+fromWei(sellerFinalEthBal)).toFixed(2)) // trans eth to seller
    ...
    it("Should transform ETH to all bidder", async function() {
      ...
      await marketplace.connect(addr2).bid(1, {value: toWei(10)});
      await marketplace.connect(addr3).bid(1, {value: toWei(12)});
      await marketplace.connect(addr4).bid(1, {value: toWei(13)});
